          ...
          <Card key={index} className={`${shift.bgColor} ${shift.borderColor} border-2 hover:shadow-lg transition-all duration-300`}>
            <CardContent className="p-4">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h3 className={`font-bold text-lg ${shift.textColor}`}>{shift.name}</h3>
                  <p className="text-xs text-muted-foreground">{shift.period}</p>
                </div>
                <Clock className={`h-5 w-5 ${shift.iconColor}`} />
              </div>
              
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-muted-foreground">Eficiência</span>
                  <span className={`font-bold text-lg ${shift.textColor}`}>
        ...
        {highlightCards.map((card, index) => (
          <Card key={index} className="hover:shadow-lg transition-all duration-300 border-primary/10">
            <CardContent className="p-4 text-center">
              <card.icon className={`h-8 w-8 mx-auto mb-2 text-${card.color}-600`} />
              <h3 className="font-semibold text-sm text-muted-foreground mb-1">{card.title}</h3>
